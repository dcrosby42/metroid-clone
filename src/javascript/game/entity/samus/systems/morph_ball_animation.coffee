...
class MorphBallAnimationSystem extends BaseSystem
  @Subscribe: [ 'morph_ball', 'samus', 'animation' ]

  process: ->
    animation = @getComp('animation')
    samus = @getComp('samus')
    # morphBall = @getComp('morph_ball')
    # morphBall.get('post') # TODO

    oldState = animation.get('state')

    newState = if samus.get('direction') == 'left'
      'roll-left'
    else
      'roll-right'
...
module.exports = MorphBallAnimationSystem
